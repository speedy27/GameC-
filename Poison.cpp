 ...
 * Description : Implémentation de Poison
 */

#include "Poison.h"
#include <iostream>

Poison::Poison(const std::string& name, int hp, const Attack& attack)
    : Pokemon(name, hp, attack) { types = { Type::Poison }; }

void Poison::attaquer(Pokemon& cible) {
    double mult=1.0;
...
Pokemon* Poison::clone() const { return new Poison(*this); }
